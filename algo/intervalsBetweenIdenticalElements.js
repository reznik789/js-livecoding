 ...
 * @return {number[]}
 */
const getDistancesSquare = (arr) => {
  const [_, res] = arr.reduce(
    ([map, res], num, idx) => {
      if (map[num] === undefined) {
        ...
        res[idx] = 0;
        map[num].forEach((otherIdx) => {
          res[otherIdx] += Math.abs(otherIdx - idx);
          res[idx] += Math.abs(idx - otherIdx);
        });
        map[num].push(idx);
      }
...
console.log(getDistancesSquare([2, 1, 3, 1, 2, 3, 3]));
